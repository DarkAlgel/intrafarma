...
{{-- ⭐️ x-data está no topo para englobar toda a funcionalidade Alpine ⭐️ --}}
<div x-data="{ openModal: {{ $errors->any() ? 'true' : 'false' }} }"> 
    
    {{-- AQUI COMEÇA O CONTEÚDO PRINCIPAL (QUE SERÁ DESLOCADO PELO app.blade.php) --}}
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            {{-- Nome (Ordenável) --}}
                            <th class="table-header p-0">
                                <a href="{{ route('fornecedores.index', ['sort' => 'nome', 'direction' => $sort === 'nome' && $direction === 'asc' ? 'desc' : 'asc']) }}" 
                                   class="flex items-center p-3 text-gray-700 hover:text-purple-600 font-semibold transition duration-150 w-full">
                                    <i class="fas fa-user mr-2"></i>Nome
                                    <i class="fas fa-sort-{{ $sort === 'nome' ? ($direction === 'asc' ? 'up' : 'down') : 'down' }} ml-2 text-sm {{ $sort === 'nome' ? 'text-purple-600' : 'text-gray-400' }}"></i>
                                </a>
                            </th>
                            
                            {{-- Tipo (Ordenável) --}}
                            <th class="table-header p-0">
                                <a href="{{ route('fornecedores.index', ['sort' => 'tipo', 'direction' => $sort === 'tipo' && $direction === 'asc' ? 'desc' : 'asc']) }}" 
                                   class="flex items-center p-3 text-gray-700 hover:text-purple-600 font-semibold transition duration-150 w-full">
                                    <i class="fas fa-tags mr-2"></i>Tipo
                                    <i class="fas fa-sort-{{ $sort === 'tipo' ? ($direction === 'asc' ? 'up' : 'down') : 'down' }} ml-2 text-sm {{ $sort === 'tipo' ? 'text-purple-600' : 'text-gray-400' }}"></i>
                                </a>
                            </th>
                            
                            {{-- Contato (Ordenável) --}}
                            <th class="table-header p-0">
                                <a href="{{ route('fornecedores.index', ['sort' => 'contato', 'direction' => $sort === 'contato' && $direction === 'asc' ? 'desc' : 'asc']) }}" 
                                   class="flex items-center p-3 text-gray-700 hover:text-purple-600 font-semibold transition duration-150 w-full">
                                    <i class="fas fa-phone mr-2"></i>Contato
                                    <i class="fas fa-sort-{{ $sort === 'contato' ? ($direction === 'asc' ? 'up' : 'down') : 'down' }} ml-2 text-sm {{ $sort === 'contato' ? 'text-purple-600' : 'text-gray-400' }}"></i>
                                </a>
                            </th>
                        </tr>
                    </thead>
    ...
    {{-- START: ESTRUTURA DO MODAL DE LUXO (O modal pode e deve ficar fora do container principal) --}}
