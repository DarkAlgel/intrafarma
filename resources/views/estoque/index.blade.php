...
{{-- ⭐️ x-data no topo para inicializar o Alpine (necessário para o modal/filtros, se houvesse) ⭐️ --}}
<div x-data="{ someState: false }"> 
    
    {{-- O Layout Mestre (app.blade.php) agora é responsável por:
         1. Renderizar a Sidebar w-64.
         2. Aplicar a classe flex-1 flex flex-col md:ml-64 no container que injeta este código. 
         3. Apenas Header e Main devem ser injetados aqui.
    --}}
            ...
            <form method="GET" action="{{ route('estoque.index') }}" class="flex items-center space-x-2 w-full md:w-3/5 lg:w-2/5">
                ...
                <div class="relative w-full shadow-md rounded-full bg-white border-2 border-purple-100 hover:border-purple-300 transition duration-300">
                    <input 
                        type="text" 
                        name="search" 
                        class="w-full py-2 pl-4 pr-24 text-gray-800 bg-transparent border-none focus:ring-0 rounded-full" 
                        placeholder="Buscar medicamento ou código..." 
                        value="{{ $searchTerm ?? '' }}" 
                    >
                    
                    {{-- ⭐️ START: ÍCONES INTERNOS (Limpar e Pesquisar) ⭐️ --}}
                    <div class="absolute right-0 top-0 h-full flex items-center pr-2">
                        
                        @if ($searchTerm)
                            {{-- ÍCONE DE LIMPEZA: Discreto, clicável, usando um ícone de X elegante --}}
                            <a href="{{ route('estoque.index') }}" 
                               class="text-xl text-gray-500 hover:text-red-600 p-2 transition duration-150" 
                               title="Limpar Pesquisa">
                                <i class="fas fa-times-circle"></i>
                            </a>
                        @endif
                        
                        {{-- ÍCONE DE PESQUISA: O botão de submit --}}
                        <button type="submit" 
                                class="h-full w-12 text-xl text-purple-600 hover:text-purple-800 transition duration-150 ml-1" 
                                title="Pesquisar">
